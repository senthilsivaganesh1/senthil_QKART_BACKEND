...
//       req.body.quantity
//     );  
//     console.log('cartCont',cart)
//     const cartWithProduct = cart.cartItems.find((cartItem)=> cartItem.product._id == productId );
//     const updatedQuantity = cartWithProduct.quantity;
//     if(updatedQuantity > 0){
//       res.status(httpStatus.OK).send(cart);
//     }else if(updatedQuantity == 0){
//       const deletedCart = await cartService.deleteProductFromCart(
//         req.user,
//         req.body.productId        
//       ); 
 ...
 */
 const checkout = catchAsync(async (req, res) => {
  ...
  return (
    res
      .status(httpStatus.NO_CONTENT)  //second test case for checkout.test.js
      .send()
  );
});
