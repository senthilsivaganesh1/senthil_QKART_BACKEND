...
const register = catchAsync(async (req, res) => {
  ...
  const token = await tokenService.generateAuthTokens(createdUser);

  const result = {"user":createdUser, "tokens":token}
...
const login = catchAsync(async (req, res) => {
  ...
  const createdUser = await authService.loginUserWithEmailAndPassword(email, password);
  const token = await tokenService.generateAuthTokens(createdUser);
  const result = {"user":createdUser, "tokens":token};
